...
OBJECTS = start.o gdt.o system.o main.o vga.o idt.o isrs.o irq.o timer.o kb.o uart.o k_heap.o page_enable.o paging.o acpi.o
default: start.asm main.c
	nasm -f elf32 -g start.asm
	nasm -f elf32 -g page_enable.asm
	i686-elf-gcc -I./include $(CFLAGS) acpi.c -o acpi.o
	...
	i686-elf-ld -T linker.ld -o kernel.elf $(OBJECTS)
	rm $(OBJECTS)
...
iso: kernel.elf
	sudo cp ./kernel.elf ./iso/boot
	qemu-system-x86_64 -m 64M -hda disk.img -s -S &
	gdb ./kernel.elf -x gdbcommands
